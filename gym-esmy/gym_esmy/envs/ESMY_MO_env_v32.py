...
class EsmyMoV32(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self,**kwargs):
        print("Initializing the EsmyMo32", flush=True)
        ...
        self.RE_installed_init = 0.0
        self.RE_installed = self.RE_installed_init
        self.elec_LT_heat_init = 0.0
        self.elec_LT_heat = self.elec_LT_heat_init
        self.elec_HT_heat_init = 0.0
        self.elec_HT_heat = self.elec_HT_heat_init
        self.fperc_BEV_init = 0.0
        self.fperc_BEV = self.fperc_BEV_init
        ...
        self.min_RE_installed = 0
        self.max_RE_installed = 1

        self.min_elec_LT_heat = 0
        self.max_elec_LT_heat = 1

        self.min_elec_HT_heat = 0
        self.max_elec_HT_heat = 1

        self.min_fperc_BEV = 0
        self.max_fperc_BEV = 1

        self.max_it = len(self.ampl_pre.years_opti)

        obslow = np.array([self.min_gwp, self.min_cost, self.min_RE_installed,
            self.min_elec_LT_heat, self.min_elec_HT_heat, self.min_fperc_BEV])
        obshigh = np.array([self.max_gwp, self.max_cost, self.max_RE_installed,
            self.max_elec_LT_heat, self.max_elec_HT_heat, self.max_fperc_BEV])
        ...
        self.max_allow_fossil = [1]
        self.min_allow_fossil = [0]
...
#------------------------------------------------------------------------------#
    ...
    def reset(self):
        ...
        return self._scale_obs(np.array([self.cum_gwp, self.cum_cost,self.RE_installed,
            self.elec_LT_heat,self.elec_HT_heat,self.fperc_BEV], dtype=np.float32))
...
#------------------------------------------------------------------------------#
    ...
    def _get_observation(self):
        ...
        last_year = self.curr_years_wnd[-1]
        ...
        if not (self.solve_result == 'solved'):
            ...
            elec_cons = self.ampl_obj.vars['elec_cons']
            self.elec_LT_heat = elec_cons[last_year,"HEAT_LOW_T"].value()/energy_cons[last_year,"HEAT_LOW_T"].value()
            self.elec_HT_heat = elec_cons[last_year,"HEAT_HIGH_T"].value()/energy_cons[last_year,"HEAT_HIGH_T"].value()
    
            prod_BEV = self.ampl_obj.vars['prod_BEV']
            EUD_mob_private = self.ampl_obj.vars['EUD_mob_private']
            self.fperc_BEV = prod_BEV[last_year].value()/EUD_mob_private[last_year].value()

        F = self.ampl_obj.vars['F']
        f_max = self.ampl_obj.params['f_max']
        share_RE_installed = 0.0
        for tech in self.ampl_obj.sets['RE_TECH']:
            share_RE_installed += F[last_year,tech].value()/f_max[last_year,tech]
        self.RE_installed = share_RE_installed/len(self.ampl_obj.sets['RE_TECH'])
        ...
        self.file_observation.write(' {:.1f}'.format(self.RE_installed))
        self.file_observation.write(' {:.1f}'.format(self.elec_LT_heat))
        self.file_observation.write(' {:.1f}'.format(self.elec_HT_heat))
        self.file_observation.write(' {:.1f}'.format(self.fperc_BEV))
    ...
    def _get_reward(self):
        if not (self.solve_result == 'solved'):
            ...
            if self.it < self.max_it - 1:
                if self.target_2035 < self.gwp_per_year['YEAR_2035'] and self.it >= 2:
                    status_2050 = 'Failure'
                    reward = 100/(self.it+1) * (self.target_2035-self.gwp_per_year['YEAR_2035'])/self.target_2035
                    done = 1
                else:
                    reward = 0
                    done = 0
            else :
                reward = 100/(self.it+1) * (self.target_2050-self.gwp_per_year['YEAR_2050'])/self.target_2050 + 50/(self.it+1) * (self.cost_budget-self.cum_cost)/self.cost_budget
                if self.target_2050 < self.gwp_per_year['YEAR_2050']:
                    status_2050 = 'Failure'
                else:
                    status_2050 = 'Success'
                done = 1
    ...
    def _get_action_to_ampl(self,action):
        ...
        # allow_foss_res = dict.fromkeys(self.ampl_obj.sets['some_NRE_RESOURCES'])
        # allow_foss_res ['ELECTRICITY']          = act_foss[0]
        # allow_foss_res ['COAL']                 = act_foss[1]
        # allow_foss_res ['GAS']                  = act_foss[2]
        # allow_foss_res ['LFO']                  = act_foss[3]
        # allow_foss_res ['H2']                   = act_foss[4]
        # allow_foss_res ['METHANOL']             = act_foss[5]
        # allow_foss_res ['AMMONIA']              = act_foss[6]
        
        # value : dict(tuple: float)
        # self.ampl_obj.set_params('allow_foss_res',allow_foss_res)
        allow_foss = act_foss[0]
