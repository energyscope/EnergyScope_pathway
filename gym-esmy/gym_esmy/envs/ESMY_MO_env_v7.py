...
class EsmyMoV7(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self,**kwargs):
        print("Initializing the EsmyMoV7", flush=True)
        ...
        self.gwp_cost_max = 1000 #€/ton_CO2_eq
        ...
        self.max_sub_renew = [0.5]
        self.min_sub_renew = [0.0]

        self.max_gwp_tax = [1.0]
        self.min_gwp_tax = [0.0]
        
        self.sub_renew = dict.fromkeys(self.ampl_obj_0.sets['YEARS'],0.0)
        self.gwp_tax = dict.fromkeys(self.ampl_obj_0.sets['YEARS'],0.0)

        actlow = np.array(self.min_allow_fossil + self.min_sub_renew + self.min_gwp_tax)
        acthigh = np.array(self.max_allow_fossil + self.max_sub_renew + self.max_gwp_tax)
...
#------------------------------------------------------------------------------#
    ...
    def reset(self):
        ...
        self.sub_renew = dict.fromkeys(years,0.0)
        self.gwp_tax = dict.fromkeys(years,0.0)
        self.ampl_obj = AmplObject(self.mod_1_path, self.mod_2_path, self.dat_path, self.ampl_options, type_model = self.type_of_model)
...
#------------------------------------------------------------------------------#
    ...
    def _get_reward(self):
        if not (self.solve_result == 'solved'):
            ...
            if self.it < self.max_it - 1:
                ...
                # reward = 200/(self.it+1) * (1-max(self.cum_gwp/self.carbon_budget, 0.75)) + 20/(self.it+1) * (-self.cum_cost)/(self.cost_budget/10)
                # reward = 200 * (1-max(self.cum_gwp/self.carbon_budget, 1)) + 20 * (-self.cum_cost)/(self.cost_budget/10)
                reward = 200 * min((self.carbon_budget-self.cum_gwp)/self.carbon_budget,0) + 200 * (self.cost_budget-self.cum_cost)/(self.cost_budget)
                if self.carbon_budget < self.cum_gwp:
                    status_2050 = 'Failure'
                else:
                    status_2050 = 'Success'
    ...
    def _get_action_to_ampl(self,action):
        ...
        sub_renew_wnd = action[-2]
        sub_renew_wnd = dict.fromkeys(years_wnd,sub_renew_wnd)
        gwp_tax_wnd = action[-1]
        gwp_tax_wnd = dict.fromkeys(years_wnd,gwp_tax_wnd*self.gwp_cost_max/1000)
        
        self.sub_renew.update(sub_renew_wnd)
        self.gwp_tax.update(gwp_tax_wnd)
        
        years_wnd_up_to = set(self.ampl_obj.sets['YEARS_WND'].copy()+
                              self.ampl_obj.sets['YEARS_UP_TO'].copy())
        
        re_tech = self.ampl_obj.sets['RE_TECH']

        
        for y in years_wnd_up_to:
            self.ampl_obj.set_params('gwp_cost',{y:self.gwp_tax[y]})
            for tech in re_tech:
                new_value = self.ampl_obj.params['c_inv'][(y,tech)]*(1-self.sub_renew[y])
                self.ampl_obj.set_params('c_inv',{(y,tech):new_value})
