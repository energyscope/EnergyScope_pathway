...
class EsmyMoV3(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self,**kwargs):
        print("Initializing the EsmyMo3", flush=True)
    ...
    def _get_reward(self):
        if not (self.solve_result == 'solved'):
            ...
            if self.it < self.max_it - 1:
                ...
                reward = 100/(self.it+1) * (self.carbon_budget-self.cum_gwp)/self.carbon_budget + 10/(self.it+1) * (self.cost_budget-self.cum_cost)/self.cost_budget
                if self.carbon_budget < self.cum_gwp:
                    status_2050 = 'Failure'
                else:
                    status_2050 = 'Success'
