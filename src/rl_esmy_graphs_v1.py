...
def pdf_generator(df_learning, output_path, type_graph = 'act'):
    list_year = ['2020','2025','2030','2035','2040']
    sectors = ['ELECTRICITY','HEAT_HIGH_T','HEAT_LOW_T','MOBILITY_PASSENGER','MOBILITY_FREIGHT','NON_ENERGY','Allow fossil']
    palette_full ={"Failure": "C0", "Success": "C1", "Failure_imp": "k"}
    clip_dict = {0:[0.1,0.2], 1:[0.1,0.2],2:[0.1,0.2],3:[0.1,0.2],4:[0.1,0.2],5:[0.1,0.2], 6:[0,1]}
    n_act = len([i for i in list(df_learning) if 'act' in i])
    for i in df_learning['batch'].unique():
        ...
        if type_graph == 'act':
            ...
            fig.suptitle('Actions over time and learning - Limit energy consumption/Fossil resources')
            for j in range(n_act):
                for k in df_learning['step'].unique():
                    ...
                    # palette = {k: palette_full[k] for k in }
                    sns.kdeplot(ax=ax,data = data_step,x='act_{}'.format(j+1), hue ='status_2050', palette=palette, legend=False, clip=clip_dict[j])
                    ax.set_yticks([])
                    ax.set(xlabel=sectors[j],ylabel=list_year[k])
                    ax.label_outer()
