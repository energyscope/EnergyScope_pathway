...
def pdf_generator(df_learning, output_path, env, type_graph = 'act'):
    ...
    df_learning.loc[df_learning['status_2050']!='Success','status_2050'] = 'Failure'
    for i in [67]:#df_learning['batch'].unique():
        ...
        if type_graph == 'act':
            ...
            for j in range(n_act):
                for k in df_learning['step'].unique():
                    ...
                    sns.kdeplot(ax=ax,data = data_step,x='act_{}'.format(j+1), legend=False, clip=xticks[j],multiple='stack')#, hue ='status_2050', palette=palette_full,
                    # sns.kdeplot(ax=ax,data = data_step,x='act_{}'.format(j+1), legend=False, clip=xticks[j],multiple='stack')
                    ax.set_yticks([])
            ...
            for j in range(2):
                ...
                for k in df_learning['step'].unique():
                    ...
                    sns.kdeplot(ax=ax,data = data_step,x=to_plot, legend=False,multiple='stack')
                    ax.set_yticks([])
            ...
            for j in range(n_obs):
                ...
                for k in df_learning['step'].unique():
                    ...
                    for n in m:
                        ax = axes[n,j]
                        sns.kdeplot(ax=ax,data = data_step,x=obs[j]+'_{}'.format(list_year[n]), legend=False, clip = [0,1],multiple='stack') #, hue ='status_2050', palette=palette
                        ...
                        # sns.kdeplot(ax=ax,data = data_step,x=obs[j]+'_{}'.format(list_year[n]), legend=False,multiple='stack')
                        # ax.set_xlim(clip)
                        ax.set_xlim([0,1])
                        ax.set_yticks([])
                    # ax.set_xticks([0,1])
...
def gif(output_path, type_graph, type_distr = 'cum'):
        if type_graph in ['sp','kde']:
            ...
            cmd = 'ffmpeg -r 10 -i {} -vcodec mpeg4 -y {}mov_'+type_graph+'.mp4'
