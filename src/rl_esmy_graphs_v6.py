        ...
        if type_graph == 'act':
            ...
            for j in range(n_act):
                for k in df_learning['step'].unique():
                    ...
                    sns.kdeplot(ax=ax,data = data_step,x='act_{}'.format(j+1), hue ='status_2050', palette=palette, legend=False, clip=xticks[0][j],multiple='stack')
                    ax.set_yticks([])
...
def reward_plus_plus(df_learning, output_path, type_graph = 'act'):
    ...
    if type_graph == 'act':
        ...
        for i in data_updated['batch'].unique():
            data = data_updated.loc[df_learning['batch']<=i]
            fig, axes = plt.subplots(len(list_year), len(x_to_plot), figsize=(15, 5))
            fig.suptitle('End-of-episode reward of successfull transition')
...
def gif(output_path, type_graph, type_distr = 'cum'):
        ...
        elif type_graph in ['ln_act','pdf_act','pdf_cum_gwp', 'pdf_cum_cost','pdf_obs','pdf_rew_act','pdf_rew_cost_gwp']:
