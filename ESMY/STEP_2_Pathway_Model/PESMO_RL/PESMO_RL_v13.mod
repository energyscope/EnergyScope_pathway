...
subject to limit_use_gas{y in YEARS_WND diff YEARS_FOR_CONSTR}:
	sum {t in PERIODS} (F_t [y,'GAS', t] * t_op [t]) <= allow_gas * 142000; # 142000 GWh is the resulting consumption of fossil NG in 2020 given by EnergyScope

subject to limit_use_lfo{y in YEARS_WND diff YEARS_FOR_CONSTR}:
	sum {t in PERIODS} (F_t [y,'LFO', t] * t_op [t]) <= allow_lfo * 159300; # 159300 GWh is the resulting consumption of LFO in 2020 given by EnergyScope

subject to limit_use_coal{y in YEARS_WND diff YEARS_FOR_CONSTR}:
	sum {t in PERIODS} (F_t [y,'COAL', t] * t_op [t]) <= allow_coal * 26400; # 26400 GWh is the resulting consumption of coal in 2020 given by EnergyScope
