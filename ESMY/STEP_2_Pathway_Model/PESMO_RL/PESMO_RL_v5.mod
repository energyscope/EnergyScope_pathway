...
subject to comp_energy_cons_trans {y in YEARS_WND diff YEAR_ONE}:
	energy_cons_trans [y] = sum {t in PERIODS, i in LAYERS, s in {"MOBILITY_PASSENGER","MOBILITY_FREIGHT"}, ss in END_USES_TYPES_OF_CATEGORY[s], tech in TECHNOLOGIES_OF_END_USES_TYPE[ss]: layers_in_out[y, tech,i] < 0} (-layers_in_out[y, tech,i]*F_t [y,tech, t] * t_op [t]);
