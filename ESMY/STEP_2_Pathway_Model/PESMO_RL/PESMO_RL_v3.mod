...
# set some_NRE_RESOURCES := {'ELECTRICITY', 'COAL', 'GAS', 'LFO','H2','METHANOL','AMMONIA'};

# param allow_foss_res {some_NRE_RESOURCES} >=0.0, <= 1.0 default 1.0; # Parameter to symbolise the action to forbid (or to allow) the use of each non-renewable resource
...
subject to limit_use_fossil{y in YEARS_WND diff YEARS_FOR_CONSTR}:
	sum {t in PERIODS, res in NRE_RESOURCES} (F_t [y,res, t] * t_op [t]) <= allow_foss * sum {i in ENERGY_MIX, t in PERIODS} (F_t [y,i, t] * t_op [t]);
