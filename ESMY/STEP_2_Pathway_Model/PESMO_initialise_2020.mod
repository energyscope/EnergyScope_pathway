...
subject to elec_prod_NUCLEAR_2020: sum {t in PERIODS}   (   F_t ['YEAR_2020','NUCLEAR', t] * t_op[t] ) = 34400; #from SPF_Economie, versus 31700 from ELIA, versus 34883 from EU
# subject to elec_prod_CCGT_2020: sum {t in PERIODS} (        F_t ['YEAR_2020','CCGT', t] * t_op[t] ) = 26800; #from SPF_Economie, versus 27 800 from ELIA, versus 27602 from EU
subject to elec_prod_WIND_OFFSHORE_2020: sum {t in PERIODS} ( F_t ['YEAR_2020','WIND_OFFSHORE', t] * t_op[t] ) = 6989; #versus 6700 from ELIA, versus 6826 from EU (SPF_Economie gives 12.8 TWh for all the wind)
subject to elec_prod_WIND_ONSHORE_2020: sum {t in PERIODS}  ( F_t ['YEAR_2020','WIND_ONSHORE' , t] * t_op[t])  = 5811; #from SPF_Economie, versus 4100 from ELIA, versus 4052 from EU (SPF_Economie gives 12.8 TWh for all the wind)
subject to elec_prod_PV_2020: sum {t in PERIODS} (F_t ['YEAR_2020','PV', t] * t_op[t] ) = 5100; #from SPF_Economie, versus 4300 from ELIA, versus 4327 from EU
subject to elec_prod_GEOTHERMAL_2020: sum {t in PERIODS} (F_t ['YEAR_2020','GEOTHERMAL', t] * t_op[t] ) = 0; #from EU
subject to elec_prod_HYDRO_RIVER_2020: sum {t in PERIODS} (F_t ['YEAR_2020','HYDRO_RIVER', t] * t_op[t] ) = 353; #from EU (confirmed by SPF_Economie, 300)
subject to elec_prod_COAL_US_2020: sum {t in PERIODS} (     F_t ['YEAR_2020','COAL_US', t] * t_op[t] ) = 1900; #from SPF_Economie
subject to elec_prod_COAL_IGCC_2020: sum {t in PERIODS} (   F_t ['YEAR_2020','COAL_IGCC', t] * t_op[t] ) = 0; #As done by GL for 2015
subject to no_elec_import_2020: sum {t in PERIODS} (   F_t ['YEAR_2020','ELECTRICITY', t] * t_op[t] ) = 0; #from SPF_Economie
...
subject to ned_prod_METHANOL_2020: sum {t in PERIODS} (F_t ['YEAR_2020','METHANOL', t] * t_op[t] ) = end_uses_input['YEAR_2020',"NON_ENERGY"] * share_ned ['YEAR_2020',"METHANOL"]; #Only import of methanol in 2020 to supply methanol
subject to ned_prod_HVC_2020: sum {t in PERIODS} (F_t ['YEAR_2020','OIL_TO_HVC', t] * t_op[t] ) = end_uses_input['YEAR_2020',"NON_ENERGY"] * share_ned ['YEAR_2020',"HVC"]; #Only Naphtha/LPG cracking to supply HVC
subject to ned_prod_AMMONIA_2020: sum {t in PERIODS} (F_t ['YEAR_2020','HABER_BOSCH', t] * t_op[t] ) = end_uses_input['YEAR_2020',"NON_ENERGY"] * share_ned ['YEAR_2020',"AMMONIA"]; #Only Haber-Bosch cracking to supply ammonia
