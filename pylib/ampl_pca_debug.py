...
class AmplPCADebug:
    ...
    def graph_PC_j(self,j,quantity,True_val = False):
        ...
        if quantity in ['Assets','Assets_eud']:
            if True_val:
                fig = px.bar(df_to_plot,x='Years',y='True_value',color='Technologies',color_discrete_map=self.color_dict_full)        
            else:
        ...
        title = '<b>Top-5 components of PC_{}_{}_{}</b>'.format(j,quantity,self.case_study)
        if True_val:
            self.custom_fig(fig,title=title,yvals = [-2,0,2],xvals=['2025','2030','2035','2040','2045','2050'],neg_value=True,type_graph = 'bar')
        else:
        ...
        fig.write_image(os.path.join(self.folder_name,'Graphs','PC_{}.pdf'.format(j)), width=1200, height=550)
        fig.write_html(self.folder_name+"/Graphs/PC_{}.html".format(j))
    ...
    def _scale_per_sector_eud_3(self, eud = None, assets = None):
        dict_tech = self._group_tech_per_eud()
    ...
    def _scaling_factor_sector_2(self):
        ...
        df = df.loc[df['Years'] != 'YEAR_2020']
        
        df['Years'] = df['Years'].str.replace('YEAR_','')
        
        
        order_entry = ['ELECTRICITY','HEAT_LOW_T','HEAT_HIGH_T',
                       'MOBILITY_PASSENGER','MOBILITY_FREIGHT',
                       'METHANOL','AMMONIA','HVC']
        
        df['End_uses_input'] = pd.Categorical(
            df['End_uses_input'], order_entry)
        df.sort_values(by=['Years','End_uses_input'], axis=0, 
                               ignore_index=True, inplace=True)
        
        title = 'Share of end-use sectors'
        
        fig = px.bar(df,x='Years',y='Share_eud',color='End_uses_input',
               color_discrete_map=self.color_dict_full)
        
        xvals = np.sort(df.Years.unique())
        yvals = [0,1]
        
        self.custom_fig(fig,title,yvals,xvals=xvals,type_graph='bar')
        fig.write_image(os.path.join(self.folder_name,'Graphs','Share_EUD.pdf'), width=1200, height=550)
        plt.close()
    ...
    def graph_PC_projection(self,case_studies,output_folder_cs,output_folder_bc):
        ...
        # for PC in ['PC_5']:#PC_transition.PC_tran.unique():
            ...
            for i, cs in enumerate(case_studies):
                ...
                assets_scaled = assets_scaled[sorted(assets_scaled.columns)]
                
                PC_temp_2 = PC_temp.set_index('Technologies').Val.to_frame().T
                # PC_temp_2 = PC_temp_2[sorted(PC_temp_2.columns)]
    ...
    def _most_different_PC(self):
        ...
        print('Sum_var for case {} = {} '.format(self.case_study,sum_var))
        ...
        while counter <= threshold_counter * sum_var:
            ...
            ind_anti_sim = np.where(similarity_matrix[:,0] <= -threshold_similarity)
            temp_sim += list(ind_sim[0])
            temp_anti_sim = list(ind_anti_sim[0])
            PC_dict[i] = dict()
            PC_dict[i][1] = var_PC_assets.index[temp_sim]
            PC_dict[i][-1] = var_PC_assets.index[temp_anti_sim]
            
            share_var = (var_PC_assets.iloc[temp_sim].sum()[0] + var_PC_assets.iloc[temp_anti_sim].sum()[0])/sum_var
            
            print('The share of total variance for case {}, PC_{} = {} '.format(self.case_study,i+1,share_var))
            
            counter += var_PC_assets.iloc[temp_sim].sum()[0] + var_PC_assets.iloc[temp_anti_sim].sum()[0]
            var_dict[i] = var_PC_assets.iloc[temp_sim].sum()[0] + var_PC_assets.iloc[temp_anti_sim].sum()[0]
            temp = temp_sim + temp_anti_sim
        ...
        sorted_value_index = np.argsort(list(var_dict.values()))
        PC_dict_final = dict()
        for i in range(len(PC_dict)):
            PC_dict_final[i] = PC_dict[sorted_value_index[-i-1]]
            
        
        self.PC_transition_dict = PC_dict_final
    ...
    def _PC_transition(self):
        ...
        for i in PC_transition_dict:
            df_pca_temp = df_pca.loc[PC_transition_dict[i][1]]
            if len(PC_transition_dict[i][-1]) > 0:
                df_pca_temp = df_pca_temp.append(-df_pca.loc[PC_transition_dict[i][-1]])
    ...
    def graph_PC_transition(self):
        ...
        xvals = np.sort(PC_transition.PC_tran.unique())
        xvals_fig = [x.replace("_","") for x in xvals]
        
        # df_to_plot = df_to_plot.sort_values(['PC_tran', 'Abs_value'],ascending= [True, False])
        
        df_to_plot['Normalized_Value'] = df_to_plot.groupby('PC_tran')['Abs_value'].transform(lambda x: x / x.sum() * 100)
        
        
        fig = px.bar(df_to_plot,x='PC_tran',y='Normalized_Value',color='Technologies',
                     color_discrete_map=self.color_dict_full,
                     category_orders = {'PC_tran':xvals},barmode="relative")
        title = 'PCs of the transition - {}'.format(self.case_study)
        self.custom_fig(fig,title=title,yvals = [0,100],xvals=xvals,type_graph = 'bar')
        fig.update_layout(
            xaxis = dict(
                ticktext = list(xvals_fig)
                ))
        pio.show(fig)
        fig.write_image(os.path.join(self.folder_name,'Graphs','PC_transition.pdf'), width=1200, height=675)
        fig.write_html(self.folder_name+"/Graphs/PC_transition.html")
        
        fig = px.bar(df_to_plot,x='PC_tran',y='Val',color='Technologies',
                     color_discrete_map=self.color_dict_full,barmode='relative',
                     category_orders = {'PC_tran':xvals})
        title = '<b>PCs of the transition - {}</b>'.format(self.case_study)
        self.custom_fig(fig,title=title,yvals = [round(-y_neg,2),0,round(y_pos,2)],xvals=xvals,type_graph = 'bar',neg_value=True)
        fig.update_layout(
            xaxis = dict(
                ticktext = list(xvals_fig)
                ))
        pio.show(fig)
        fig.write_image(os.path.join(self.folder_name,'Graphs','PC_transition_True_Val.pdf'), width=1200, height=675)
        fig.write_html(self.folder_name+"/Graphs/PC_transition_True_Val.html")
        
        A = 4
    
    def graph_PC_over_the_years(self):
        file_name = os.path.join(self.folder_name,self.case_name,'_PCA.p')
        file_name = open(file_name,"rb")
        
        results = pkl.load(file_name)
        file_name.close()
        ...
        df = results['df_pca_PC'].copy()
        df['Expl_Var_Ratio'] = 100*df['Expl_Var_Ratio']
        df['Text'] = df['Expl_Var_Ratio'].apply(lambda x: '%.1f' % x)
        df['Years'] = df['Years'].str.replace('YEAR_', '')
        fig = px.bar(df,x='Years',y='Expl_Var_Ratio',color='PC',text='Text',
                     color_discrete_sequence = grays)
        fig.update_traces(textfont=dict(family="Rawline", size=18))
        ...
        yvals = [0,90,100]
        xvals = ['2025','2030','2035','2040','2045','2050']
        title = '<b>Share of explained variance per PC, per year</b><br>[%]'
        
        self.custom_fig(fig,title=title,yvals = yvals,xvals=xvals,type_graph = 'bar')
        
        fig.add_shape(x0=-0.75,x1=5.4,
                  y0=90,y1=90,
                  type='line',layer="below",
                  line=dict(color='rgb(90,90,90)', width=1),opacity=0.5)
        
        fig.write_image(os.path.join(self.folder_name,'Graphs','PC_over_the_years.pdf'), width=1200, height=550)
        fig.write_html(self.folder_name+"/Graphs/PC_over_the_years.html")
    ...
    def _remove_outliers(self,df):
        ...
        for column in df.columns:
            ...
            threshold = 1.5
            outliers_high = df[(df[column] > Q3 + threshold * IQR)]
            outliers_low = df[(df[column] < Q1 - threshold * IQR)]
            
            if not(outliers_high.empty):
                df.loc[outliers_high.index, column] = Q3 + threshold * IQR
            if not(outliers_low.empty):
                df.loc[outliers_low.index, column] = Q1 - threshold * IQR
    ...
    def dict_color(category):
        ...
        if category == 'Electricity':
            ...
            color_dict = {"ELECTRICITY" : "deepskyblue", "HEAT_HIGH_T":"red","HEAT_LOW_T_DECEN":"lightpink", "HEAT_LOW_T_DHN":"indianred","HEAT_LOW_T":"indianred", "MOB_PUBLIC":"gold", "MOB_PRIVATE":"goldenrod","MOBILITY_PASSENGER":"gold","MOBILITY_FREIGHT":"darkgoldenrod", "NON_ENERGY": "darkviolet", "INFRASTRUCTURE":"grey","HVC":"cyan",'STORAGE':'chartreuse', 'OTHERS':'gainsboro'}
    ...
    def _pca_assets_transition(self):
        ...
        B = A.sort_values(by=['W_value'],ascending=False)
        B = B.nlargest(30,'W_value')
        B.reset_index(inplace=True)
        title = 'Top-20 features over the transition for {} - W_value'.format(self.case_study)
        fig = px.bar(B,x='Technologies',y='W_value')
        fig.update_layout(title=title)
        
        pio.show(fig)
        
        B = A.sort_values(by=['Value'],ascending=False)
        B = B.nlargest(30,'Value')
        B.reset_index(inplace=True)
        title = 'Top-20 features over the transition for {} - Value'.format(self.case_study)
        fig = px.bar(B,x='Technologies',y='Value')
        fig.update_layout(title=title)
